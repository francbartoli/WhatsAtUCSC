...
// PSM Mods
function handleOurAddFeed() {
    // check to see if we have feeds
    // if we have feeds show them
    if(localStorage["feeds"]) {
        return JSON.parse(localStorage["feeds"]);
    } else {
        // if we don't have feeds, use our This Week feed
    var feedname = "This Week";
	var feedurl = "http://www1.ucsc.edu/news_events/calendar/rss/ThisWeek.ashx";  
        }
	//var feedname = $.trim($("#feedname").val());
	//var feedurl = $.trim($("#feedurl").val());
	...
	if(errors != "") {
		...
		addFeed(feedname, feedurl);
        // need to figure out what page to land on
		$.mobile.changePage("choose_events.html");
	}
}